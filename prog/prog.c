...
static ssize_t mydev_read(struct file *filp, char *buffer, size_t length, loff_t *offset)
{
    ...
    for (i = 0; i < length && *msg_ptr; i++) {
        if (put_user(*(msg_ptr++), buffer + i)) {
            return -EFAULT;
        }
        bytes_read++;
    }
...
static ssize_t mydev_write(struct file *filp, const char *buffer, size_t length, loff_t *offset)
{
    ...
    for (i = 0; i < length && (msg_ptr - msg < BUF_LEN - 1); i++) {
        if (get_user(*(msg_ptr++), buffer + i)) {
            return -EFAULT;
        }
        bytes_written++;
    }
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Team from CS-2119");
MODULE_DESCRIPTION("A virtual character device driver");
